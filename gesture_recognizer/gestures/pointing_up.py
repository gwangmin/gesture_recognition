...
class PointingUp(OneHandGestureBase):
    '''
    This class represents Pointing_Up Gesture.
    ...
    def check(self, handedness_name: str, hand_landmarks: List[landmark_module.NormalizedLandmark],
        ...
        if self.state == self.AVAILABLE_STATES[0]:
            # if pointing_up
            if info['gesture_name'] == GestureRecognizer.POINTING_UP:
                return True
...
def pointing_up():
    '''
    Pointing_Up gesture recognition with webcam.
    '''
    # Settings
    max_num_hands = 2
    download_model = True
    ...
    pu_mgr = TwoHandGestureManager({'Left': PointingUp(), 'Right': PointingUp()})
        ...
        if success:
            ...
            if result.gestures:
                for i in range(len(result.hand_landmarks)):
                    ...
                    if pu_mgr.check(handedness_name, hand_landmarks, info):
                        pu_mgr.handler(handedness_name)
                    # draw landmarks
