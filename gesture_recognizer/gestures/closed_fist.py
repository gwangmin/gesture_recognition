...
class ClosedFist(OneHandGestureBase):
    '''
    This class represents Closed_Fist Gesture.
...
def closed_fist():
    '''
    Closed_Fist gesture recognition with webcam.
    '''
    # Settings
    max_num_hands = 2
    download_model = True
    ...
    cf_mgr = TwoHandGestureManager({'Left': ClosedFist(), 'Right': ClosedFist()})
        ...
        if success:
            ...
            if result.gestures:
                for i in range(len(result.hand_landmarks)):
                    ...
                    if cf_mgr.check(handedness_name, hand_landmarks, info):
                        cf_mgr.handler(handedness_name)
                    # draw landmarks
