...
class ThumbUp(OneHandGestureBase):
    '''
    This class represents Thumb_Up Gesture.
    ...
    def check(self, handedness_name: str, hand_landmarks: List[landmark_module.NormalizedLandmark],
        ...
        if self.state == self.AVAILABLE_STATES[0]:
            # if thumb_up
            if info['gesture_name'] == GestureRecognizer.THUMB_UP:
                return True
...
def thumb_up():
    '''
    Thumb_Up gesture recognition with webcam.
    '''
    # Settings
    max_num_hands = 2
    download_model = True
    ...
    tu_mgr = TwoHandGestureManager({'Left': ThumbUp(), 'Right': ThumbUp()})
        ...
        if success:
            ...
            if result.gestures:
                for i in range(len(result.hand_landmarks)):
                    ...
                    if tu_mgr.check(handedness_name, hand_landmarks, info):
                        tu_mgr.handler(handedness_name)
                    # draw landmarks
