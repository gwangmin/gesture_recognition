...
class ILoveYou(OneHandGestureBase):
    '''
    This class represents ILoveYou Gesture.
    ...
    def check(self, handedness_name: str, hand_landmarks: List[landmark_module.NormalizedLandmark],
        ...
        if self.state == self.AVAILABLE_STATES[0]:
            # if iloveyou
            if info['gesture_name'] == GestureRecognizer.ILOVEYOU:
                return True
...
def iloveyou():
    '''
    ILoveYou gesture recognition with webcam.
    '''
    # Settings
    max_num_hands = 2
    download_model = True
    ...
    ily_mgr = TwoHandGestureManager({'Left': ILoveYou(), 'Right': ILoveYou()})
        ...
        if success:
            ...
            if result.gestures:
                for i in range(len(result.hand_landmarks)):
                    ...
                    if ily_mgr.check(handedness_name, hand_landmarks, info):
                        ily_mgr.handler(handedness_name)
                    # draw landmarks
