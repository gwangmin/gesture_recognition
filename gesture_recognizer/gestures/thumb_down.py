...
class ThumbDown(OneHandGestureBase):
    '''
    This class represents Thumb_Down Gesture.
    ...
    def check(self, handedness_name: str, hand_landmarks: List[landmark_module.NormalizedLandmark],
        ...
        if self.state == self.AVAILABLE_STATES[0]:
            # if thumb_down
            if info['gesture_name'] == GestureRecognizer.THUMB_DOWN:
                return True
...
def thumb_down():
    '''
    Thumb_Down gesture recognition with webcam.
    '''
    # Settings
    max_num_hands = 2
    download_model = True
    ...
    td_mgr = TwoHandGestureManager({'Left': ThumbDown(), 'Right': ThumbDown()})
        ...
        if success:
            ...
            if result.gestures:
                for i in range(len(result.hand_landmarks)):
                    ...
                    if td_mgr.check(handedness_name, hand_landmarks, info):
                        td_mgr.handler(handedness_name)
                    # draw landmarks
